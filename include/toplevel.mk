...
export MAKE_PARALLEL_JOBS=$(filter -j%,$(MAKEFLAGS))
...
prepare-tmpinfo: FORCE
	$(INSTR_QUIET)+$(MAKE) -r -s $(STAGING_DIR_HOST)/.prereq-build $(PREP_MK)
...
.config: ./scripts/config/conf $(if $(CONFIG_HAVE_DOT_CONFIG),,prepare-tmpinfo)
	$(INSTR_QUIET)if [ \! -e .config ] || ! grep CONFIG_HAVE_DOT_CONFIG .config >/dev/null; then \
...
scripts/config/%onf: FORCE
	$(INSTR_QUIET)$(_SINGLE)$(SUBMAKE) $(if $(findstring s,$(OPENWRT_VERBOSE)),,-s) \
...
defconfig: scripts/config/conf prepare-tmpinfo FORCE
	touch .config
	$(INSTR_QUIET)if [ ! -s .config -a -e $(HOME)/.openwrt/defconfig ]; then cp $(HOME)/.openwrt/defconfig .config; fi
	[ -L .config ] && export KCONFIG_OVERWRITECONFIG=1; \
...
ifeq ($(wildcard $(STAGING_DIR_HOST)/bin/quilt),)
  prepare_kernel_conf:
	$(INSTR_QUIET)+$(SUBMAKE) -r tools/quilt/compile
else
  prepare_kernel_conf: ;
endif
...
$(STAGING_DIR_HOST)/.prereq-build: include/prereq-build.mk
	mkdir -p tmp
	$(INSTR_QUIET)$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f $(TOPDIR)/include/prereq-build.mk prereq 2>/dev/null || { \
		echo "Prerequisite check failed. Use FORCE=1 to override."; \
		false; \
	}
  ifneq ($(realpath $(TOPDIR)/include/prepare.mk),)
	$(INSTR_QUIET)$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f $(TOPDIR)/include/prepare.mk prepare 2>/dev/null || { \
...
printdb: FORCE
	$(INSTR_QUIET)$(_SINGLE)$(NO_TRACE_MAKE) -p $@ V=99 DUMP_TARGET_DB=1 2>&1
...
download: .config FORCE $(if $(wildcard $(STAGING_DIR_HOST)/bin/flock),,tools/flock/compile) $(if $(wildcard $(STAGING_DIR_HOST)/bin/zstd),,tools/zstd/compile)
	$(INSTR_QUIET)+$(foreach dir,$(DOWNLOAD_DIRS),$(SUBMAKE) $(dir);)

clean dirclean: .config
	$(INSTR_QUIET)+$(SUBMAKE) -r $@

prereq:: prepare-tmpinfo .config
	$(INSTR_QUIET)+$(NO_TRACE_MAKE) -r -s $@

check: .config FORCE
	$(INSTR_QUIET)+$(NO_TRACE_MAKE) -r -s $@ QUIET= V=s
...
%::
	$(INSTR_QUIET)+$(PREP_MK) $(NO_TRACE_MAKE) -r -s prereq
	$(INSTR_QUIET)./scripts/config/conf $(KCONF_FLAGS) --defconfig=.config Config.in
	$(INSTR_QUIET)+$(ULIMIT_FIX) $(SUBMAKE) -r $@
...
%::
	...
	$(INSTR_QUIET)+$(ULIMIT_FIX) $(SUBMAKE) -r $@ $(if $(WARN_PARALLEL_ERROR), || { \
...
distclean:
	...
	$(INSTR_QUIET)$(_SINGLE)$(SUBMAKE) -C scripts/config clean
