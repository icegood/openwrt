...
def localmirrors():
    ...
    if path.exists(fn):
        ...
        for line in lines:
            m = rg.match(line)
            if m is not None:
                mirrors.extend(m.groups[1].split(';'))
...
def get_mirrors(filename):
    ...
    for mirror in argv[3:]:
        for mirror_setting in LOCAL_MIRROR_SETTINGS:
            ...
            if patternmatch is not None:
                mirrors.extend([x + "/" + patternmatch.groups[1] for x in mirror_setting.get("model")])
                break
        patternmatch = re.fullmatch('^\@KERNEL\/(.+)$', mirror)
        if patternmatch is not None:
            extra = [patternmatch.groups[1]]
            if re.match(r"linux-\d+\.\d+(?:\.\d+)?-rc", filename) is not None:
                ...
                if patternmatch is not None:
                    extra.append(extra[0] + "/longterm/v" + patternmatch.groups[1])
